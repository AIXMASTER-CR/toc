...
class Main {
  public static void main(String[] args) { DFAValidateString.main(); }
}
...
class DFAValidateString {
  public static void main() {
    ...
    Set<String> validInputs =
        new HashSet<>(Arrays.asList(inputString.split(",")));
    ...
    for (String state : states) {
      ...
      for (String validInput : validInputs) {
        message = String.format(
            "Enter transition (δ) for state '%s' having input '%s': ", state,
            validInput);
...
class DFA {
  ...
  public DFA(String startingState, Set<String> finalStates,
  ...
  public void validateString(String validateString) {
    ...
    for (char c : validateString.toCharArray()) {
      Map<String, String> current_state_transition =
          transitions.get(current_state);
      String next_state = current_state_transition.get(String.valueOf(c));

      if (next_state == null) {
        Utility.show_error("Your string contains value '" + c +
                           "' which is not present in the input set");
        return;
      }
    ...
    if (finalStates.contains(current_state)) {
      ...
      Utility.show_error("Your string ends at state '" + current_state +
